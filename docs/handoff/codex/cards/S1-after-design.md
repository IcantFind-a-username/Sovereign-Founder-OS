...
S1-07 → [S1-09 CLI](S1-09.md) → [S1-08 浏览器](S1-08.md) →
[S1-10A 共享测试捕获](S1-10A.md) → [S1-10 两根隔离](S1-10.md) →
[S1-11 独立验收](S1-11.md)。这些卡已冻结，实际依赖完成状态见 backlog。
10A 单文件完善测试进程捕获；10 只消费受验收 helper，避免同卡混合两组职责。
沿用 standalone-v2 边界；完整业务/AI MVP 仍需后续阶段。
