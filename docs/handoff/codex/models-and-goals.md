...
~~~text
目标：持续开发、集成并实际验证 Sovereign Founder OS 的全流程咨询业务 MVP，
交付到创始人能亲自全链路测试的状态，让一个人在同一个本地界面中完成：
...
主动精简上下文，严格执行 protocol §9.5.1。
当前工作上下文只保留目标引用、当前卡、适用约束、必要接口和最近诊断；
按需读取文件的相关段落，完整日志落到获准路径，不反复加载整仓库或聊天历史。
每张卡交接、阻塞、阶段结束以及可观察到的上下文压力出现时，
在现有获准报告中保存精简恢复摘要，保留 Git/任务/attempt/失败/预算/锁与证据引用。
分派子任务使用独立上下文和最小完整简报，保留模型显式覆盖，不复制全部会话。
若平台提供可调用的压缩机制，先保存摘要再使用；否则依赖平台自动压缩，
不能把写一份摘要或新开线程谎称为已完成平台压缩。
压缩或中断恢复后，核对目标、最新用户指令、持久记录、实际 HEAD 和活动任务再继续；
已处理的追加要求不重做，未完成的继续；不重开零失败 Goal、不复用已终结的 attempt；
未终结 attempt 沿用原编号与累计预算，不能因压缩或恢复清零。
每轮必须推进一个可核验的产品/实现步骤或报告具名阻塞，
不能连续用重读、重新规划、摘要或文档提交替代施工。
...
8. 交付时提供可运行版本、启动命令、演示步骤、实际操作截图或录像、
   ...
   同时给出可照做的创始人全链路测试清单：准备条件、点击步骤、预期结果、
   失败/返工触发方法、重置与重新启动方式；亲自在真实浏览器跑过同一清单。
   入口必须指向验收版本且可打开，不将已停止的服务描述为正在运行；
   交付为“可供创始人测试”，不能代替创始人声称已完成实际用户验收。
...
普通技术问题在获准范围内自行修复，必要设计交强模型解决；
不在每卡结束后请求重复批准，不因一次工具失败、上下文压缩或阶段完成就终结目标。
所有可独立推进的工作都已耗尽而仍需外部条件时，保存恢复摘要，
按平台当前规则处理 Goal 阻塞，明确解除条件；不能循环空跑或标记已完成。
在本目标范围内自动完成可逆实现与受审本地集成；
保留已有用户改动，不自动 push、创建 PR、发布、部署、真实发信或操作资金。

只有上述全流程在统一界面实际走通、最终验收通过并可供我亲自全链路测试，
...
上下文精简与恢复的唯一规则见 [protocol §9.5.1](protocol.md#context-continuity)。自动压缩由平台提供；官方配置项 `model_auto_compact_token_limit` 未设置时采用模型默认值。当前不新增压缩配置或承诺主线程可调用未暴露的压缩工具，也不把模型上下文窗口大小改成虚构值。[OpenAI Docs 配置参考](https://learn.chatgpt.com/docs/config-file/config-reference)
