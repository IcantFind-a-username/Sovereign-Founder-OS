...
**目标规格：Target；尚未启动。**本节是当前完整 MVP Goal 的唯一正文，替换原先分别止于 S0/S1 的启动提示词。验收定位为合成业务数据、真实模型参与、真实可操作界面的集成 MVP；真实客户数据与真实外部执行仍按 S4/S5 的门槛推进。
...
3. 需求分析员、报价助理、交付规划员和质量检查员通过受控模型通道实际运行，
