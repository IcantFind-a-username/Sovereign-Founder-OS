...
Owner 最新要求：“替luna划分他可以做的小任务……你验收你派发，这样循环”。
据此采用以下监督模式，优先于下文自动运行器的执行前置；S0 自动 harness
保持未验收状态，不再阻塞产品施工。产品安全 RFC 和真实测试要求不变。

1. 主控按 backlog 选择一个小任务，冻结精确写集、接口和验收命令后提交认领。
2. 普通实现用 Luna medium；架构判断、验收和兜底用 Astra high。记录实际
   分派与候选，不要求尚未实现的 S0 事件运行器或历史迁移先通过。
3. 每项实现完成先停止 worker，主控审查实际 diff 和测试；通过后登记结果并
   派下一项。修改过候选的人不能把自己的改动当独立审查。
4. 同一卡两次实质失败后停止 Luna，保留诊断交 Astra；环境阻塞不伪装失败。
5. 卡/报告/backlog 与 Git 是当前证据；旧 S0 错误历史保留且不得宣称通过。
   不再增加恢复卡。恢复自动模式需独立证明其运行能力。
6. 每次交接只保留当前卡、候选、检查、失败数和下一步；以可运行产品增量
   推进，完整 MVP 仍必须满足 models-and-goals §13.1。

### 监督模式的任务分流

S1-G02/G03及G04的实际候选表明，修改源码准入门、固定生产fixture和恶意变异
验收属于架构边界工作。后续尚未分派的这类卡直接由Astra承担，再由另一强模型
独立验收；这落实“必要设计交强模型处理”，不把安全边界裁决当普通产品施工。
业务数据、文案、页面与既定接口实现仍交Luna medium。已在执行的卡继续保留
原attempt和两次失败上限，不倒改角色历史或重新清零计数。
