...
- [x] **P1 | `docs/handoff/reports/`, `docs/backlog.md` | Phase 1 step-2: fixture reservation / exact-effect Tasks 7–11 evidence.**
  Completed 2026-09-16 on baseline `4707835`. Delivers
  [2026-09-16-runtime-phase1-step2-fixture-reservation-exact-effect-evidence.md](handoff/reports/2026-09-16-runtime-phase1-step2-fixture-reservation-exact-effect-evidence.md):
  owner-plan Tasks 7–11; 50 TSV rows green; no product Exact Effect; #152 F03
  pin still true; F04 barrier is filesystem bundle not fixture
  reserve-to-dispatch; no RP1-02/03 product claim. Docs-only queue close;
  prior report records ALL GREEN. Does not invert F03 or claim RP1-02/03.

- [x] **P1 | `docs/handoff/reports/`, `docs/backlog.md` | Phase 1 step-3: fixture evidence / browser / split / recovery / freshness Tasks 12–15.**
  Completed 2026-09-16 on baseline `0d7972c` (#154 tip). Delivers
  [2026-09-16-runtime-phase1-step3-fixture-evidence-recovery-freshness.md](handoff/reports/2026-09-16-runtime-phase1-step3-fixture-evidence-recovery-freshness.md):
  owner-plan Tasks 12–15; TSV 12/13/15 (22 rows) green; Task 14 has **zero**
  TSV rows (`--task 14` fail-closed); RFC 0007 Amendment 1 still not Accepted
  (#150 text only); product journal/SIGKILL/race/freshness-open/checkpoint/F04
  pins cited not re-implemented. No RP1-04/05/06 product claim. Docs-only
  queue close. Does not implement Amendment 1 or invert F03.
...
- [x] **P1 | `crates/capability/` (and capability tests only) | v01-D01 — Separate verification from consumption.**
  ...
  **Design Accept ≠ product Exact Effect.** No product Current, no 1C0, no
  ...
  v01-D02 and v01-D03 landed; v01-D04 is in progress below; v01-D05…D07 stay
  unclaimed in
...
- [x] **P1 | new `publish = false` fixture package + boundary script | v01-D02 — Release-excluded single-process fixture boundary.**
  ...
  **Design Accept ≠ product Current.** Not 1C0, Exact Effect, ActiveV2, or
  RP1. v01-D03 landed below; v01-D04 is in progress; v01-D05…D07 stay
  unclaimed in
  [the Wave D queue handoff](handoff/reports/2026-09-13-rfc-0002-amendment-1-wave-d-queue.md).
...
- [ ] **P1 | fixture package `sovereign-synthetic-owner-effect` + `crates/capability` issuance of the closed profile (fixture feature only) | v01-D04 — Seal intent and bridge fresh UV to RFC 0003.** IN PROGRESS (2026-09-16)
  Licensed by RFC 0002 Amendment 1. Depends on v01-D03 (`b8cadc6`). Random
  intent before content access; fixed CRLF RFC 5322 bytes inside the
  coordinator; private sealed payload; synthetic preview. `FreshUvGrant`
  (non-cloneable) → only `ApprovalBridge::approve_invocation` emits RFC 0003
  evidence. Exact Capability V2 claims use wire tokens `low_risk_effectful` +
  `core_wasm` + `write_rfc5322`. Product issuance stays `pure_compute`.
  **Design Accept ≠ product Current.** Not 1C0, Exact Effect, ActiveV2, RP1,
  dispatch, reservation, or product `low_risk_effectful` admission. Done when
  named tests cover substitution, header injection, session-alone denial,
  grant replay, epoch mismatch, restart invalidation, and compile-fail
  access; `signed_shape` / existing `pure_compute` tokens stay byte-stable.
...
- [x] **P1 | `crates/authority/` | Make bundle consumption one recoverable transaction.**
...
- [x] **P1 | `crates/authority/` | Add durable, fail-closed revocation records.**
...
- [x] **P2 | `apps/cli/src/` | `demo` writes its events and admission records into the owner's real data root.**
  Landed 2026-09-16 via #163: default `demo` writes under `<data-dir>/demo`; `demo_fast_leaves_seeded_product_root_byte_unchanged` pins the product ledger/vault/admissions byte-unchanged.
...
- [x] **P3 | `scripts/test_changed.sh` | The scoped gate cannot see a file included by `#[path]` from another crate.** Completed 2026-09-16 (#165).
...
- 2026-08-26: implemented RFC 0003 Amendment 1's bundle transaction in `crates/authority` — `AuthorityStore::consume_bundle` claims a token, an approval, and an idempotency key as one recoverable transaction instead of three separate claims, per Amendment 1 part (a)/(b) exactly. Bundle id is a domain-separated SHA-256 over the three subject ids plus the invocation fingerprint (deterministic, so a same-input retry resumes its own bundle instead of colliding or burning). `AuthorityRecord` gained `#[serde(default)] bundle_hex: Option<String>` (old records keep parsing). The six protocol steps (intent, revocation pre-check, token claim, idempotency bind, approval claim, revocation re-check + commit) are private methods exposed to the in-crate test module so crash points can be driven directly, exactly as the amendment requires; `claim()` was refactored onto a new generic `publish_record` helper shared with the bundle-record publish path (no behavior change, removed duplication). `purge_expired` now also purges `bundles/` entries by the same expiry rule via a small `Expiring` trait, satisfying the amendment's "stale partial claims self-expire with their subjects." The revocation pre-check/re-check against `revoked-tokens/`/`revoked-approvals/` is implemented as specified in part (a), but nothing publishes into those directories yet — `revoke_token`/`revoke_approval` are the next, still-blocked backlog entry (part c); today the check is a no-op that will start observing real revocations once that entry lands. The existing single-claim methods (`consume_token`/`consume_approval`/`bind_idempotency`) are unchanged and stay for the `crates/capability` migration's own round, per the entry's scope note. All six pinned Amendment-1-part-(e) test names pass verbatim: `a_retried_bundle_after_any_interruption_completes_without_burning_claims`, `racing_bundles_over_the_same_token_have_exactly_one_winner`, `racing_retries_of_the_same_bundle_authorize_exactly_once`, `a_reopened_store_answers_a_partial_bundle_identically`, `a_foreign_uncommitted_bundle_denies_other_consumers`, `purge_removes_expired_bundles_and_their_claims`. `cargo test -p sovereign-authority` runs 14 tests, all passing (8 pre-existing plus the 6 above). Added `sha2 = { workspace = true }` as a dependency (already pinned workspace-wide; only `Cargo.lock`'s dependency graph changed). Full gate ALL GREEN (gate-self-test, file-size, fmt, clippy(workspace), test(workspace), tsc(frontend)).
- 2026-08-27: implemented RFC 0003 Amendment 1 part (c), durable revocation, in `crates/authority`. `AuthorityStore` gained `revoked_tokens`/`revoked_approvals` directories (already-named constants, now actually opened and populated) and `revoke_token`/`revoke_approval`, both routing through one private `revoke()` that publishes an exclusive-create record via the existing `publish_record` primitive and returns a three-way `RevocationOutcome` (`Revoked` / `AlreadyRevoked` / `RevokedAfterConsumption`), matching part (c) verbatim. "Already authorized" is judged precisely, not just "claim exists": a new `subject_is_authorized` helper treats a legacy claim (no `bundle_hex`) as immediately authorized, but a bundle-claimed subject only once its bundle's `.committed` marker exists — a token claimed by an in-flight, uncommitted bundle is not yet an authorized effect, so revoking it reports the clean `Revoked` outcome, not `RevokedAfterConsumption`; this distinction is exercised directly in `revoking_a_consumed_claim_reports_the_distinct_outcome`. `bundle_check_revocation` (steps 2 and 6) and the legacy `consume_token`/`consume_approval` (defense in depth) now share one `check_not_revoked` free function that fails closed on both a present record and an unparsable one (`CorruptRecord`, never silently allowed). `purge_expired` purges the two revocation directories on the same expiry rule as everything else. All four pinned Amendment-1-part-(e) revocation test names pass verbatim: `a_revoked_token_fails_closed_across_reopen`, `revoking_a_consumed_claim_reports_the_distinct_outcome`, `a_revoke_vs_consume_race_ends_in_one_durable_outcome`, `a_corrupt_revocation_record_fails_closed`; the race test asserts the outcome pairing is exhaustively one of exactly two combinations (bundle authorized + `RevokedAfterConsumption`, or bundle denied `Revoked` + revoke reports `Revoked`) across 20 randomized runs, never any other pairing. `cargo test -p sovereign-authority` runs 18 tests, all passing. Before adding this code, moved the crate's `#[cfg(test)] mod tests { … }` out of `lib.rs` into `src/tests.rs` (declared as `#[cfg(test)] mod tests;`) as a pure, test-verified no-op refactor — `lib.rs` was at 1033 lines against the 1200 god-file limit and this round's addition would have pushed it over; see the new CLAUDE.md lesson. No new dependencies. Full gate ALL GREEN (gate-self-test, file-size, fmt, clippy(workspace), test(workspace), tsc(frontend)).
