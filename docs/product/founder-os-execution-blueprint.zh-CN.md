...
**创始人已确认首个试点市场为新加坡。**第一类用户仍是独立顾问/专业服务一人公司，先验证一条从客户需求到报价、交付与应收的完整路径。欧盟、美国保留为后续覆盖方向，具体成员国和州尚未确定；中国大陆留待独立本地化评估。确认试点方向不表示已有设计伙伴、获客或付费证据，也不是设立公司或法律适用意见。

比较前提：下载到本机的业务工具、用户自带模型凭据、少量受邀的独立顾问；先合成数据，真实资料必须经过 S4 放行。首轮不加入自动签约、付款、诉讼代理、招聘筛选或信用决策。邀请制、开源、本地运行、自带 key 均不是普遍法律豁免。

| 次序与决策状态 | 试点范围 | 本项目判断与需核对事项 |
| --- | --- | --- |
| 1，已确认 | 新加坡：同一类咨询业务的少量设计伙伴 | 先集中一种工作流、一个主要司法辖区，检验报价与交付是否有用。PDPA 仍需评估；PDPC 对 AI 开发、部署和服务商角色已有[官方指引](https://www.pdpc.gov.sg/-/media/files/pdpc/pdf-files/advisory-guidelines/advisory-guidelines-on-the-use-of-personal-data-in-ai-recommendation-and-decision-systems.pdf)。指引自身说明其非约束性，不能全部编码成法定义务。 |
| 2，后续建议 | 美国：先确定一个州和同类客户 | 用相同业务流程验证第二个市场；州级义务与适用门槛须逐项确认。例如 [CCPA](https://oag.ca.gov/privacy/ccpa) 有特定适用条件，不能把它当作所有美国小企业均适用的全国规则，也不能据未达某一门槛推断其他义务不存在。 |
| 3，后续建议 | 欧盟：先选一个成员国 | 具体国家优先由可接触的设计伙伴决定，暂未选择。需评估 GDPR 和 AI Act 下实际产品角色、用途及适用义务；参考 [GDPR 第 3 条](https://eur-lex.europa.eu/legal-content/EN/ALL/?uri=CELEX%3A02016R0679-20160504)及[欧委会 AI 透明度指南](https://digital-strategy.ec.europa.eu/en/policies/guidelines-ai-transparency-obligations)。欧盟不等于一份覆盖全部成员国事务的规则包。 |
...
后续若要调整首个市场，由创始人依据真实获客证据、支持成本和准入条件作出新决定，再修改本节；worker 不自行改变已确认方向。不得虚构市场规模、转化率、法规评分或“最宽松国家”排名。目标是先获得可重复的业务使用证据，再扩大地区覆盖。
...
首批五个岗位可以共用一个 Claude 模型，通过职责、输入范围、技能和输出契约区分；不要求五套模型、五份凭据或五个永不结束的进程。产品中的员工配置与开发用 Luna/reviewer 配置分开，开发角色仍以 [models-and-goals](../handoff/codex/models-and-goals.md) 为唯一规范。

每份技能包至少包含版本与负责人、适用任务、输入/输出 schema、操作步骤、模板、正反例、固定评测、失败/转人工条件和可请求的工具清单。技能版本绑定本次任务及结果；工具清单只表达需求，实际权限由 OS 独立裁决。技能若携带脚本，按插件/工件准入和沙箱规则执行；提示词中的技能说明不能绕过权限。可借鉴 [Anthropic Agent Skills](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview) 的能力包组织方式，但不默认启用服务商托管的文件或代码执行环境，也不自动上传公司技能、附件与知识库。

**MVP 不做后训练。**先固定任务契约，再改善输入、提示词、技能、工具和 RAG，用同一评测集观察错误。只有出现稳定、重复的能力缺口，并拥有经授权、清理与审阅的数据及独立留出评测，才评估微调或其他后训练是否优于较轻方案。API 可调用不等于具备该模型的训练权限；训练不能替代当前法律、价格和客户事实的来源更新，不能提供执行授权。后续实验需记录质量、成本和退回基线的条件。技能市场、热插拔组织图和自动招聘同样后置。
...
**MVP 先交付本地 Web 工作台；产品目标是本地桌面应用。**当前 `sovereign ui` 只提供已有预览，绑定 `127.0.0.1`，默认端口 7787；这不是完整 MVP 已经可用的证明。S1 独立 Playground 及后续实验入口沿用各自冻结规范；最终集成必须提供一个创始人可连续操作的入口，而非要求在多个演示之间手工搬运数据。

先复用现有前端和 Rust 业务能力，形成客户、报价、AI 任务、法务引用、审阅和交付的完整页面。桌面封装在浏览器 MVP 可用后评估 Tauri：复用 Web UI，由受限的原生接口接入本地服务与系统凭据。浏览器、WebView 或插件不能取得凭据明文或任意原生调用权；安装包与 loopback 地址都不证明数据隔离或 owner 身份已成立。

首个安装包方向为 macOS 的 `.app`/`.dmg`，Windows 的 `.exe`/`.msi` 后续验证；格式只是发行形式，签名、更新、卸载、凭据生命周期与恢复需独立验收。参考 [Tauri 的 DMG](https://v2.tauri.app/distribute/dmg/) 与 [Windows 安装器](https://v2.tauri.app/distribute/windows-installer/) 文档。安装包、移动 App 和多租户云端 SaaS 不列为本轮全流程浏览器 MVP 的完成条件。
...
| **S3 合成 AI 员工实验** | 真实模型完成分析/起草/检查的限定任务 | S3-M 先满足真实模型调用的协议和实际授权依赖；与确定性基线对比，输出、成本、故障与边界都有证据 | 独立实验面；真实模型不因此获得真实业务数据或凭据。 |
| **S4 真实本地业务 Alpha** | 创始人录入真实客户与工作记录，保存/恢复，审阅草稿 | owner、Vault 激活、恢复、存储清单与 RFC 0004 门槛全部成立 | 不以警告框、fixture token、标志位代替门槛。 |
| **S5 一个真实受控业务外部动作** | 一次明确批准的发送或业务集成 | 复用已验收的凭据/效果机制，扩展确切业务动作、审计与核对路径 | 模型 API 出网已是外部效果，不能等到 S5 才承认；收件人和内容变更使授权失效；Indeterminate 不自动重发。 |
...
### 7.1 Anthropic 首个模型连接与凭据保管

**已选方向：使用创始人已有的 Claude API 作为首个产品模型提供者；实际模型 ID、额度和可调用资格在接入时核实。当前尚未配置或验证连接。**[现有 model 源码](../../crates/model/src/lib.rs)明确只有确定性替身，真实 egress broker 尚不存在。现有 Wasm 沙箱也不自动约束外部 Python/PydanticAI 进程的文件、环境变量、网络或 IPC；需要在真实运行器上证明这些边界。

凭据采用可替换的本地保管后端与受限 broker：macOS 首选系统 Keychain；未来其他平台另行选择系统保管服务。模型、技能、普通 worker、前端和日志不接收永久 secret，受信任核心操作只通过不透明句柄引用；只有获准 broker 的必要内部操作使用明文，不能提供通用“读出 key”工具。凭据按 provider/account/purpose 分配、撤销与轮换，同一提供者下的多个员工复用受控调用路径。业务 Vault 的数据加密/恢复与 API 凭据保管是不同职责；legacy `vault.key` 与数据同根的实现、尚未产品激活的 Vault v2 都不能被描述为已经可用的凭据代理。

具体录入步骤、本地条目命名和配置状态只维护在[模型凭据配置指南](../handoff/codex/provider-setup.md)。本地保存 key 不代表系统已能读取、API 已连接、已允许任意调用或可发送真实公司资料。GitHub repository secret 用于相应 Actions 注入场景，不是可通过读取 API 取回明文的本地保管后端；不得绕 CI 导出 secret 作为配置手段。

**真实模型依赖前置到 S3-M。**即使输入是合成数据或已批准的公开法律片段，真实 API 请求仍是 RFC 0004 所定义的外部效果；S3 编号不构成豁免。架构卡必须先制定并审阅真实 egress / Exact Effect 所需协议，检查 Program 1C0、Capability V2、durable authority、受保护协调状态及执行记录的实际接线，并复用原 backlog 中对应任务。未满足的必要机制须先落地，不能用 RFC 0006 fixture、环境变量或 SDK 直连冒充通过；也不能把完整真实客户存储功能一并塞进首个模型卡。S3-M 卡仍为设计方向，精确安全协议和接口冻结后才允许 Luna 实现。

首次连接验收先使用非秘密测试值验证 Keychain 访问隔离，再经真实 owner 授权对明确的合成输入做受限调用；用户能看见提供者、模型、实际发送内容与已批准的调用上限。锁定目标和允许的网络路径，禁用绕过协议的 SDK 自动重试/自动换提供者，保留无凭据、拒绝访问、失效凭据、超时、超限及 `Indeterminate` 的区别。实测只报告无秘密的状态和可得用量，不回显 key、请求认证头、敏感配置或完整 provider 错误。该验收完成后 S3-01 的框架实验才能使用真实 Claude；真实客户数据和业务发送仍分别走 S4/S5。
