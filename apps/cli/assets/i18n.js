...
const STRINGS = {
  en: {
    ...
    cc_documents: "Documents", cc_documents_meta: (c) => c.drafts + " draft · " + c.approved_pending_delivery + " to send · " + c.delivered + " delivered",
    ...
    cc_chain: "Audit chain", cc_signed: "Signed sends", cc_signed_meta: "each send carries signed evidence",
    ...
    doc_reopen_hint: "Editing reopens this as a new draft revision; the rejection or revocation stays on the audit chain.",
    ...
    ws_no_approvals: "Nothing awaiting approval. Sending a document always stops here for a decision, made in this app on this device. This preview cannot yet verify who makes it — any program on this machine can reach the same decision.",
    ws_approve: "Approve", ws_reject: "Reject",
    ws_approval_for: "Send request for: ",
    ws_export: "Export all my data", ws_export_note: "Company, customers, documents, and the signed audit chain, as one JSON file.",
    ...
    ws_verify_state: (c) => [
      (c.venture_present ? "company profile" : "no company profile"),
      c.customers + " customers", c.documents + " documents",
      c.approvals + " approvals (" + c.signed_approvals + " signed)",
      c.projects + " projects", c.tasks + " tasks", c.follow_ups + " follow-ups",
      c.payments + " payments", c.employees + " employees", c.decisions + " decisions",
      c.compliance_reports + " compliance reports", c.disclosures + " model disclosures",
    ].join(" · "),
    ws_verify_contents_note: "Contents are an inventory, not a verification: the signed chain covers the recorded events, so it cannot tell you a restore dropped rows. Compare these counts against the workspace you expected.",
    ...
    vault_meta: "encrypted at rest (AES-256-GCM); key stored beside the data", plugins_meta: "owner-signed admission records",
    ...
    skip_reason: (r) => ({
      ...
      failed: "tried, returned an error",
      red_data_confidentiality: "refused — red data may not leave this device",
      raw_request_is_local_only: "refused — a raw request may only go to a local provider",
      cloud_labelled_denied: "refused — a cloud-labelled provider never sees a raw request",
    }[r] || r),
    skip_not_recorded: "reason not recorded",
    stayed_local: "local (self-reported)", left_device: "left device (self-reported)",
    ...
    gauntlet_intro: "Ten attacks against the running kernel: token replay, input substitution, executing without the owner's admission, hostile in-host compilation, cache poisoning, a manifest demanding host capabilities, an infinite-loop plugin, red-zone data exfiltration, approval bypass, and audit tampering.",
    loading: "loading…",
    ...
    footer_limits: "approving a send records your decision on this device, and this preview cannot yet verify who clicked — any local program can; the vault's key is stored beside the data it encrypts; the app performs no network send, so \"delivered\" is a marker you set by hand; replay defense is process-local; demo trust anchors are not production key management.",
    attacks: {}
  },
  zh: {
    title: "SOVEREIGN FOUNDER OS",
    ...
    cc_documents: "文档", cc_documents_meta: (c) => c.drafts + " 份草稿 · " + c.approved_pending_delivery + " 份待你发送 · " + c.delivered + " 份已送达",
    ...
    cc_chain: "审计链", cc_signed: "已签名发送", cc_signed_meta: "每次发送都有签名证据",
    ...
    doc_reopen_hint: "编辑会把它重新打开为新的草稿版本;拒绝或撤销的记录留在审计链上。",
    ...
    ws_no_approvals: "暂无待审批事项。发送文档永远会先停在这里等待一个决定,由本机上的这个应用作出。本预览版尚无法验证是谁作出的——本机上任何程序都能触达同一个决定。",
    ws_approve: "批准", ws_reject: "拒绝",
    ws_approval_for: "发送申请:",
    ws_export: "导出我的全部数据", ws_export_note: "公司、客户、文档与签名审计链,打包为一个 JSON 文件。",
    ...
    ws_verify_state: (c) => [
      (c.venture_present ? "有公司资料" : "无公司资料"),
      c.customers + " 位客户", c.documents + " 份文档",
      c.approvals + " 项审批(其中 " + c.signed_approvals + " 项已签名)",
      c.projects + " 个项目", c.tasks + " 项任务", c.follow_ups + " 条跟进",
      c.payments + " 笔收款", c.employees + " 名员工", c.decisions + " 条决定",
      c.compliance_reports + " 份合规报告", c.disclosures + " 条模型披露",
    ].join(" · "),
    ws_verify_contents_note: "这里是清点,不是验证:签名链覆盖的是已记录的事件,因此它无法告诉你某次恢复丢了数据。请把这些数量和你预期的工作区对照。",
    ws_verify_pass_word: "通过", ws_verify_fail_word: "未通过",
    tile_device: "设备身份", tile_events: "审计事件", tile_vault: "保险库条目", tile_plugins: "已准入插件",
    vault_meta: "静态加密(AES-256-GCM);密钥与数据同处一地", plugins_meta: "所有者签名的准入记录",
    ...
    skip_reason: (r) => ({
      unhealthy: "没有应答",
      failed: "调用了,返回错误",
      red_data_confidentiality: "已拒绝 —— 红色数据不得离开本机",
      raw_request_is_local_only: "已拒绝 —— 原始请求只能发给本机提供方",
      cloud_labelled_denied: "已拒绝 —— 标为云端的提供方不得接收原始请求",
    }[r] || r),
    skip_not_recorded: "未记录原因",
    stayed_local: "留在本机(自报)", left_device: "离开本机(自报)",
    sec_plugins: "已准入插件 — 内容寻址存储",
    sec_ledger: "审计记录 — 签名哈希链(最近 25 条)",
    sec_gauntlet: "安全闯关 — 真实攻击,即刻在内存中运行",
    btn_run: "运行安全闯关", btn_refresh: "刷新状态",
    gauntlet_intro: "对运行中的内核发起十项攻击:令牌重放、输入替换、未经所有者准入的执行、宿主内恶意编译、缓存投毒、清单越权申请、死循环插件、红区数据外泄、绕过审批、篡改审计。",
    loading: "加载中…",
    ...
    footer_limits: "批准发送会在本机记录你的决定,而本预览版尚无法验证是谁点的——本机任何程序都能点;保险库的密钥与它加密的数据同处一地;应用不执行任何网络发送,因此“已投递”只是你手动标记的;重放防御仅限进程内;演示信任锚并非生产级密钥管理。",
    attacks: {
