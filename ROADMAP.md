...
| **Model Gateway** | Current prototype: an Experimental deterministic routing simulator plus an Experimental loopback Ollama adapter, not a real-model sandbox. Raw requests no longer follow caller Amber/Green onto a cloud-labelled provider; an adapter's self-reported `local` flag cannot authorize raw Protected input. Remaining [RFC 0004](rfcs/0004-data-sovereignty-boundaries.md) work: compiler-owned public projection dispatch, sandboxed local model, presets/queue. This is not v0.2 complete. |
