...
pub const GENESIS_HASH: &str = "0".repeat(64);
...
impl AuditLedger {
    ...
    pub fn from_events(events: Vec<AuditEvent>) -> Result<Self, LedgerError> {
        let ledger = Self { events };
        ledger.verify_chain()?;
        Ok(ledger)
    }
...
mod tests {
    ...
    fn persist_ledger() {
        ...
        let loaded = AuditLedger::load(&path).unwrap();
